/**
 * System prompt templates for each interview mode.
 * Supports user customization via Storage.
 */
...
const Prompts = {
  ...
  /** Reset a mode's prompt to default. */
  async resetTemplate(mode) {
    await Storage.remove('prompt_' + mode);
  },

  /** Build the final system prompt for coding mode. */
  ...
  /** Build the final system prompt for system design mode. */
  ...
  /** Build the final system prompt for behavioral mode. */
